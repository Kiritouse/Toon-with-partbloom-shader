Shader "Unlit/CelRenderOfBumpFinal"
{
    Properties
    {
        ...
        _BumpScale("Bump Scale",Float) = 1.0//用来缩放法线纹理的
            [Space(20)]
        ...
        pass
        {
           Tags {"LightMode" = "ForwardBase"}
            ...
            { 
                ...
                float4 T2W2:TEXCOORD4;//将法线纹理中的从切线空间转换到世界空间的TBN矩阵
            };
            ...
            v2f vert(a2v v)
            {
                ...
                o.uv.xy = v.uv.xy * _MainTex_ST.xy + _MainTex_ST.zw;//主纹理贴图的偏移缩放
                o.uv.zw = v.uv.xy * _BumpMap_ST.xy + _BumpMap_ST.zw;//法线贴图的偏移缩放
            ...
            { 
               ...
               // half3 viewDir = normalize(_WorldSpaceCameraPos.xyz - worldPos);
                ...
                float3 bump = UnpackNormal(tex2D(_BumpMap, i.uv.zw));//解开uv，bump就是经过加上法线贴图运算后重新得到的法线,此时在切线空间中
                bump.xy *= _BumpScale;
                bump.z = sqrt(1.0 - saturate(dot(bump.xy, bump.xy)));//z分量可以由xy分量获得
               ...
               // half3 diffuse = halfLambert > _ShadowRange ? _MainColor : _ShadowColor; 
                ...
                half ramp = smoothstep(0, _ShadowSmooth, halfLambert - _ShadowRange);//0代表白色，从白色到阴影的颜色生成一条平滑曲线
                ...
                half NdotL = max(0, dot(bump, worldLightDir));//光线越和发现重合，点积就越大，说明就越亮
